...
extern "C" void __cdecl OnCombatADirectPre(uintptr_t unit,uintptr_t currentSide) {
    if(!unit||!currentSide||!g_exe)return;
    uint32_t k0=0,k1=0;
...
extern "C" void __cdecl OnBattleOutcomePre(uintptr_t retAddr,uintptr_t battle,uint32_t selector) {
    if(!battle||!g_exe)return;uint32_t retRva=(uint32_t)(retAddr-(uintptr_t)g_exe);uintptr_t losing=0,winning=0,province=0;
...
extern "C" void __cdecl OnDeferEnqueuePre(uintptr_t object,uintptr_t retAddr) {
    if(!object||!g_exe)return;uint32_t k0=0,k1=0;if(!ReadU32(object+OFF_UNIT_ID0,&k0)||!ReadU32(object+OFF_UNIT_ID1,&k1))return;
...
static void __fastcall hkSharedRemove(void* self,void*,void* unit,uint32_t arg2,uint32_t arg3) {
    ...
    // Re-read after queue commit: ControllerWorker may have resolved the coalition captor
...
// ---------------- patching ----------------
static bool ValidateCall(uint32_t rva,uint32_t targetRva){uint8_t* p=g_exe+rva;__try{if(p[0]!=0xE8)return false;int32_t rel=*(int32_t*)(p+1);return p+5+rel==g_exe+targetRva;}__except(EXCEPTION_EXECUTE_HANDLER){return false;}}
static bool PatchCall(uint32_t rva,void* hook){if(g_callPatchCount>=8)return false;uint8_t* site=g_exe+rva;PatchCallRec& rec=g_callPatches[g_callPatchCount++];rec.site=site;rec.installed=false;CopyMemory(rec.original,site,5);DWORD old=0;if(!VirtualProtect(site,5,PAGE_EXECUTE_READWRITE,&old))return false;intptr_t rel=(uint8_t*)hook-(site+5);site[0]=0xE8;*(int32_t*)(site+1)=(int32_t)rel;FlushInstructionCache(GetCurrentProcess(),site,5);DWORD tmp=0;VirtualProtect(site,5,old,&tmp);rec.installed=true;return true;}
static void RestoreCallPatches(){for(int i=g_callPatchCount-1;i>=0;i--){PatchCallRec& r=g_callPatches[i];if(!r.installed||!r.site)continue;DWORD old=0;if(VirtualProtect(r.site,5,PAGE_EXECUTE_READWRITE,&old)){CopyMemory(r.site,r.original,5);FlushInstructionCache(GetCurrentProcess(),r.site,5);DWORD tmp=0;VirtualProtect(r.site,5,old,&tmp);}r.installed=false;}}
...
static bool InstallSharedHook(void* target){if(memcmp(target,SIG_SHARED,SHARED_PATCH_LEN)!=0)return false;g_sharedHook.target=target;memcpy(g_sharedHook.original,target,SHARED_PATCH_LEN);BYTE* t=(BYTE*)VirtualAlloc(0,32,MEM_COMMIT|MEM_RESERVE,PAGE_EXECUTE_READWRITE);if(!t)return false;memcpy(t,target,SHARED_PATCH_LEN);BYTE* j=t+SHARED_PATCH_LEN;j[0]=0xE9;*(int32_t*)(j+1)=(int32_t)(((BYTE*)target+SHARED_PATCH_LEN)-(j+5));g_sharedHook.trampoline=t;g_origSharedRemove=(SharedRemove_t)t;if(!write_rel_jmp(target,(void*)&hkSharedRemove,SHARED_PATCH_LEN)){VirtualFree(t,0,MEM_RELEASE);g_sharedHook.trampoline=0;g_origSharedRemove=0;return false;}g_sharedHook.installed=true;return true;}
static void UninstallSharedHook(){if(!g_sharedHook.installed)return;DWORD old=0;if(VirtualProtect(g_sharedHook.target,SHARED_PATCH_LEN,PAGE_EXECUTE_READWRITE,&old)){memcpy(g_sharedHook.target,g_sharedHook.original,SHARED_PATCH_LEN);FlushInstructionCache(GetCurrentProcess(),g_sharedHook.target,SHARED_PATCH_LEN);DWORD tmp=0;VirtualProtect(g_sharedHook.target,SHARED_PATCH_LEN,old,&tmp);}if(g_sharedHook.trampoline)VirtualFree(g_sharedHook.trampoline,0,MEM_RELEASE);g_sharedHook.installed=false;}
...
static DWORD WINAPI Worker(LPVOID){
    while(!InterlockedCompareExchange(&g_stop,0,0)){
        Sleep(100);DWORD now=GetTickCount();struct ResolveEvent{uint32_t k0,k1,captor;} ev[64];unsigned evn=0;
        EnterCriticalSection(&g_ctxLock);
...
static bool ValidateAll(){
    ...
    if(memcmp(g_exe+RVA_SHARED_REMOVE,SIG_SHARED,sizeof(SIG_SHARED))!=0)return false;
    if(memcmp(g_exe+RVA_SITE_MANUAL,SIG_MANUAL_SITE,sizeof(SIG_MANUAL_SITE))!=0)return false;
    if(memcmp(g_exe+RVA_SITE_COMBAT_A,SIG_COMBAT_A_SITE,sizeof(SIG_COMBAT_A_SITE))!=0)return false;
    if(memcmp(g_exe+RVA_SITE_COMBAT_B,SIG_COMBAT_B_SITE,sizeof(SIG_COMBAT_B_SITE))!=0)return false;
    if(memcmp(g_exe+RVA_COUNTRY_LEADER_ADD,SIG_ADD,sizeof(SIG_ADD))!=0)return false;
    if(memcmp(g_exe+RVA_COUNTRY_LEADER_REMOVE,SIG_REMOVE,sizeof(SIG_REMOVE))!=0)return false;
    return true;
}
static DWORD WINAPI InitThread(LPVOID){
    InitializeCriticalSection(&g_logLock);InitializeCriticalSection(&g_ctxLock);InitializeCriticalSection(&g_stateLock);if(!OpenLog())return 10;Sleep(250);g_exe=(uint8_t*)GetModuleHandleA(0);g_exeSize=image_size((uintptr_t)g_exe);g_originalBattleOutcome=g_exe+RVA_BATTLE_OUTCOME;g_originalDeferEnqueue=g_exe+RVA_DEFER_ENQUEUE;g_sharedRemoveEntry=g_exe+RVA_SHARED_REMOVE;g_countryLeaderAdd=(CountryLeaderAdd_t)(g_exe+RVA_COUNTRY_LEADER_ADD);g_countryLeaderRemoveAddr=g_exe+RVA_COUNTRY_LEADER_REMOVE;
    Log("[BOOT] LEADER_CAPTURE_AUTO_TRANSFER_R06B_D328 exe_base=%08X image_size=%08X policy=CAPTURE_ALL_PROVEN_COMBAT_DESTRUCTIONS_THEN_AUTO_TRANSFER",(uint32_t)g_exe,g_exeSize);
    ...
    if(!ValidateAll()){Log("[FATAL] signature/call validation failed. NO PATCHES OR GAMEPLAY WRITES INSTALLED.");return 20;}Log("[VERIFY] all mandatory signatures/calls PASS");
    bool ok=true;
    ...
    if(!ok){Log("[FATAL] hook installation failed; restoring partial hooks");RestoreCallPatches();UninstallSharedHook();return 30;}
    Log("[READY] gameplay_writes=ENABLED scope=LEADER_REGISTRY_ONLY province_writes=DISABLED owner_tuple_writes=DISABLED manual_route=IGNORE");
    ...
    g_worker=CreateThread(0,0,Worker,0,0,0);return 0;
}

BOOL WINAPI DllMain(HINSTANCE h,DWORD reason,LPVOID){
    if(reason==DLL_PROCESS_ATTACH){g_self=h;DisableThreadLibraryCalls(h);CreateThread(0,0,InitThread,0,0,0);}
    else if(reason==DLL_PROCESS_DETACH){InterlockedExchange(&g_stop,1);RestoreCallPatches();UninstallSharedHook();if(g_log!=INVALID_HANDLE_VALUE)CloseHandle(g_log);}
    return TRUE;
}
