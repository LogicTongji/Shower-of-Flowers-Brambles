...
local IntelligenceMissions = {
	AtPeace = {
		SpyMission.SPYMISSION_COUNTER_ESPIONAGE,
                SpyMission.SPYMISSION_INCREASE_THREAT,
	...
	AtWar = {
		...
		SpyMission.SPYMISSION_LOWER_NATIONAL_UNITY,
                SpyMission.SPYMISSION_COVERT_OPS
	}
}
...
function ManageSpiesAtHome()
	...
	if liNewMission == nil then
                ...
                missions[SpyMission.SPYMISSION_COUNTER_ESPIONAGE] = 1

		-- Raise our unity so we dont surrender so easy and can get special laws past
		if IntelligenceData.NationalUnity < 80 then
			missions[SpyMission.SPYMISSION_RAISE_NATIONAL_UNITY] = 3
		...
		elseif IntelligenceData.PartyPopularity < 30 then
			missions[SpyMission.SPYMISSION_BOOST_RULING_PARTY] = 3
		-- Nothing really to do but our unity is not 90 so raise it
		elseif IntelligenceData.NationalUnity < 90 then
			missions[SpyMission.SPYMISSION_RAISE_NATIONAL_UNITY] = 3
		-- Support for our party to try and get rid of fractured government
		elseif IntelligenceData.PartyPopularity < 50 then
                        missions[SpyMission.SPYMISSION_RAISE_NATIONAL_UNITY] = 1
			missions[SpyMission.SPYMISSION_BOOST_RULING_PARTY] = 3
		-- Nothing really to do but our unity is not 90 so raise it
		elseif IntelligenceData.NationalUnity < 99 then
                        missions[SpyMission.SPYMISSION_BOOST_RULING_PARTY] = 1
			missions[SpyMission.SPYMISSION_RAISE_NATIONAL_UNITY] = 3
                elseif IntelligenceData.PartyPopularity < 64 then
                        missions[SpyMission.SPYMISSION_COUNTER_ESPIONAGE] = 2
                        missions[SpyMission.SPYMISSION_RAISE_NATIONAL_UNITY] = 1
			missions[SpyMission.SPYMISSION_BOOST_RULING_PARTY] = 3
                elseif IntelligenceData.PartyPopularity < 89 then
                        missions[SpyMission.SPYMISSION_COUNTER_ESPIONAGE] = 3
                        missions[SpyMission.SPYMISSION_RAISE_NATIONAL_UNITY] = 1
			missions[SpyMission.SPYMISSION_BOOST_RULING_PARTY] = 1
		end
