...
function ProductionMinister_Tick(minister)
	...
	if ProductionData.ManpowerMobilize and ProductionData.icAvailable > 0.1 then
				...
				elseif loMilitary:IsAir() then
					for loConstDef in loMilitary:GetBrigades() do
						...
						if loSubUnit:IsCag() then
							...
							if loSubUnit:Isshipborne_attack() then
								liNeededNavalIC = liNeededNavalIC + loBuildItem:GetCost()
							else
								if loSubUnit:Isshipborne_bomber() then
									liNeededNavalIC = liNeededNavalIC + loBuildItem:GetCost()
								else
									if loSubUnit:Isseaplane() then
										liNeededNavalIC = liNeededNavalIC + loBuildItem:GetCost()
									else
		...
		if liNeededNavalIC > 0 then
			...
			local liCAGsNeeded = ProductionData.TotalCounts[UnitTypes.carrier.Index] + ProductionData.TotalCounts[UnitTypes.escort_carrier.Index]
			...
			local liShipborneAttacksNeeded = ProductionData.TotalCounts[UnitTypes.carrier.Index] + ProductionData.TotalCounts[UnitTypes.escort_carrier.Index]
			local liShipborneAttacksCount = ProductionData.TotalCounts[UnitTypes.shipborne_attack.Index]
			
			if liShipborneAttacksNeeded > liShipborneAttacksCount then
				ProductionData.UnitNeeds[UnitTypes.shipborne_attack.Index] = liShipborneAttacksNeeded - liShipborneAttacksCount
				laNavalUnitRatio["shipborne_attack"] = 0
			end
			
			local liShipborneBombersNeeded = ProductionData.TotalCounts[UnitTypes.carrier.Index]
			local liShipborneBombersCount = ProductionData.TotalCounts[UnitTypes.shipborne_bomber.Index]
			
			if liShipborneBombersNeeded > liShipborneBombersCount then
				ProductionData.UnitNeeds[UnitTypes.shipborne_bomber.Index] = liShipborneBombersNeeded - liShipborneBombersCount
				laNavalUnitRatio["shipborne_bomber"] = 0
			end
			
			local liSeaplanesNeeded = ProductionData.TotalCounts[UnitTypes.seaplane_carrier.Index]
			local liSeaplanesCount = ProductionData.TotalCounts[UnitTypes.seaplane.Index]
			
			if liSeaplanesNeeded > liSeaplanesCount then
				ProductionData.UnitNeeds[UnitTypes.seaplane.Index] = liSeaplanesNeeded - liSeaplanesCount
				laNavalUnitRatio["seaplane"] = 0
			end
		end
