...
function DiploScore_SendExpeditionaryForce(voAI, voActorTag, voRecipientTag, voObserverTag, action)
	if voObserverTag == voActorTag then
		...
		if recipientCountry:GetDailyBalance( CGoodsPool._SUPPLIES_ ):Get() > 1.0 then
			...
			if supplyStockpile > weeksSupplyUse * 640.0 then
				score = score + 1
			elseif supplyStockpile > weeksSupplyUse * 360.0 then
				score = score + 1
			end
			
			if recipientCountry:IsAtWar() and (supplyStockpile > weeksSupplyUse * 64.0) then
				score = score + 1
			else
