...
)
{
    ...
    const int rowCount = model.empty()
        ? 0
        : static_cast<int>(
            (model.size() + static_cast<size_t>(columns) - 1)
    ...
    {
        if (!item.definition || item.index >= model.size())
        {
            continue;
        }

		const GuiListItem& modelItem = model.items[item.index];
