...
$OverlayDll  = "$DebugDir\scripted_gui_overlay.dll"
...
& "$OverlayRoot\build-win\Debug\scripted_gui_injector.exe" `
    $Hoi3Exe `
    "$OverlayRoot\build-win\Debug\scripted_gui_overlay.dll" `
    $ProjectRoot `
    "-mod" `
    "mod/hoi3_scripted_gui_dev.mod"
```
...
```powershell
git diff -- overlay/src/gui_host_d3d9.cpp
```
...
6. HOI3 是否真的加载了 Shower-of-Flowers-Brambles？
        ↓
7. scripted_gui_overlay.dll 是否加载？
        ↓
